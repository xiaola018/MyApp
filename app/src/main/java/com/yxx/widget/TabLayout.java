...
import com.yxx.app.util.DpUtils;
...
//                LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
        ...
        //*****************************自定义部分 START*****************************
        bottomLineMode = a.getInt(R.styleable.TabLayout_bottomLineMode, 0);
        if (bottomLineMode == 1) {
            bottomLineColor = a.getInt(R.styleable.TabLayout_bottomLineColor, getResources().getColor(android.R.color.darker_gray));
            bottomLineHeight = a.getDimensionPixelSize(R.styleable.TabLayout_bottomLineHeight, 0);
            bottomLineWidth = a.getDimensionPixelSize(R.styleable.TabLayout_bottomLineWidth, 0);
            ...
            super.addView(linearLayout, 0, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.MATCH_PARENT));
        } else {
            ...
            if (bottomLineMode == 2) {
                ...
                mTabStrip.setBottomLineWidth(bottomLineWidth);
            }
            super.addView(mTabStrip, 0, layoutParams);
            //View view = new View(context);
            //view.setBackgroundColor(bottomLineColor);
            //linearLayout.addView(view,new LinearLayout.LayoutParams(bottomLineWidth, bottomLineHeight));
        }

        mTabStrip.setSelectedIndicatorWidth(a.getDimensionPixelSize(R.styleable.TabLayout_tabIndicatorWidth,
                DpUtils.dp2px(20)));
        mTabStrip.setSelectedIndicatorRadius(a.getDimensionPixelSize(R.styleable.TabLayout_tabIndicatorRadius,
                DpUtils.dp2px(5)));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.TabLayout_tabIndicatorColor, 0));
        mTabStrip.setDividerCorlor(a.getColor(R.styleable.TabLayout_dividerColor, getResources().getColor(android.R.color.darker_gray)));
        dividerHeight = a.getDimensionPixelSize(R.styleable.TabLayout_dividerHeight, 0);
        dividerWidth = a.getDimensionPixelSize(R.styleable.TabLayout_dividerWidth, 0);
        mTabStrip.setDividerHeight(dividerHeight);
        mTabStrip.setDividerWidth(a.getDimensionPixelSize(R.styleable.TabLayout_dividerWidth, 0));
        //*****************************自定义部分 END*****************************

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.TabLayout_tabIndicatorHeight, DpUtils.dp2px(4)));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.TabLayout_tabIndicatorColor,
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        void setDividerCorlor(int corlor) {
            if (mDividerPaint.getColor() != corlor) {
                mDividerPaint.setColor(corlor);
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }

        void setDividerHeight(int height) {
            if (mDividerHeight != height) {
        ...
        void setBottomLineWidth(int width) {
            if (mBottomLineWidth != width) {
        ...
        void setBottomLineMode(int mode) {
            if (mBottomLineMode != mode) {
                mBottomLineMode = mode;
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }
        //*****************************自定义部分 END*****************************
...
//            }
            ...
            if (bottomLineMode == 2) {
                bottomLineOffset = bottomLineHeight;
                IndicatorOffset = mSelectedIndicatorHeight;
            }
            if (mSelectedIndicatorPaint != null && mIndicatorLeft >= 0 && mSelectedIndicatorHeight > 0 && mSelectedIndicatorWidth != 0
                ...
                int a = getHeight() - mSelectedIndicatorHeight - bottomLineHeight + bottomLineOffset;
                int b = (getHeight() - bottomLineHeight + bottomLineOffset);


                //底部选中滑动线的绘制(即文字下面表示选中的哪一页)
                RectF rectF = new RectF(mIndicatorLeft + offset, getHeight() - mSelectedIndicatorHeight - bottomLineHeight + bottomLineOffset ,
                        mIndicatorRight - offset, getHeight() - bottomLineHeight + bottomLineOffset);
                //画圆角矩形
                canvas.drawRoundRect(rectF, mSelectedIndicatorRadius, mSelectedIndicatorRadius, mSelectedIndicatorPaint);
            }
            int h = getHeight();
            int c = getHeight() - bottomLineOffset;
            int d = (getHeight() - bottomLineHeight - bottomLineOffset);
            //底部分割线的绘制 对应bottomLineMode
            if (mBottomLinerPaint != null && bottomLineHeight > 0) {
                RectF rectF = new RectF(getPaddingLeft(), getHeight() - IndicatorOffset, getWidth() - getPaddingRight() , getHeight() - bottomLineHeight - IndicatorOffset);
                //画圆角举行
            ...
            if (dividerWidth > 0 && dividerHeight != 0 && getChildCount() > 1) {
                ...
                for (int i = 0; i < childCount - 1; i++) {
                    ...
                    if (dividerVOffset < 0 || dividerVOffset < 0) {
                        dividerVOffset = 0;
                    }
                    int dividerHOffset = dividerWidth / 2;
                    RectF rectF = new RectF(getChildAt(i).getRight() - dividerHOffset, dividerVOffset,
                            getChildAt(i).getRight() + dividerHOffset, getHeight() - dividerVOffset);
